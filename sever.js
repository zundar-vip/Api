...
function predict() {
    ...
    for (var i = 0; i < fullHistory.length; i++) {
        txArr.push(fullHistory[i].tx);
        totals.push(fullHistory[i].total);
    }
    ...
    var sRatio = t10 / 10, lRatio = t20 / 20;
    ...
    var patterns = {
        'ttxx': [0.7, 'x'], 'xxtt': [0.7, 't'], 'tttxxx': [0.9, 'x'], 'xxxttt': [0.9, 't'],
        'txxxt': [0.6, 't'], 'xtttx': [0.6, 'x'], 'txtx': [0.4, 't'], 'xtxt': [0.4, 'x'],
        'ttxxtt': [0.8, 'x'], 'xxttxx': [0.8, 't'], 'ttttxxxx': [0.95, 'x'], 'xxxxtttt': [0.95, 't']
    };
...
function connectWebSocket() {
    ...
    try { ws = new WebSocket(WS_URL, { headers: WS_HEADERS }); } catch(err) { reconnectTimeout = setTimeout(connectWebSocket, 2500); return; }
    ...
    ws.on('open', function() {
        ...
        pingInterval = setInterval(function() { if (ws && ws.readyState === WebSocket.OPEN) try { ws.ping(); } catch(e) {} }, 15000);
    });
    ...
    ws.on('close', function() { if (pingInterval) clearInterval(pingInterval); if (reconnectTimeout) clearTimeout(reconnectTimeout); reconnectTimeout = setTimeout(connectWebSocket, 2500); });
...
app.get('/', function(req, res) {
    var html = '<h2>Sunwin Tai Xiu AI</h2>';
    html += '<p><a href="/sun">API JSON</a></p>';
